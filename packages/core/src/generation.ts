...
export async function generateText({
  ...
  if (verifiableInference && runtime.verifiableInferenceAdapter) {
    ...
    try {
      ...
      // Verify the proof
      const isValid = await runtime.verifiableInferenceAdapter.verifyProof(
        result
      )
      if (!isValid) {
    ...
    switch (provider) {
      ...
      case ModelProviderName.ETERNALAI: {
        ...
        let system_prompt =
          runtime.character.system ?? settings.SYSTEM_PROMPT ?? undefined
        try {
          const on_chain_system_prompt = await getOnChainEternalAISystemPrompt(
            runtime
          )
...
export const generateImage = async (
  ...
  try {
    if (runtime.imageModelProvider === ModelProviderName.HEURIST) {
      ...
      console.log("prompt: " + data.prompt + " model : " + model)
